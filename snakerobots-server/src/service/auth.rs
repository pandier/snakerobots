...
pub async fn verify_session(app: &AppState, session_id: Uuid) -> eyre::Result<Option<SessionModel>> {
    Ok(get_session(app, session_id)
        .await?
        .filter(|x| Utc::now() < x.expires_at))
}

pub async fn get_session(app: &AppState, session_id: Uuid) -> eyre::Result<Option<SessionModel>> {
...
pub async fn create_session(app: &AppState, user_id: i32) -> eyre::Result<SessionModel> {
