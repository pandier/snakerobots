...
pub fn create_standard_game(robot1: Box<dyn Robot>, robot2: Option<Box<dyn Robot>>, seed: Option<u64>) -> Game {
...
pub fn create_standard_game_with_size(
...

    {
        ...
        players.push(Player::new(snake, robot1));
    }

    if let Some(robot2) = robot2 {
        let mut snake = Snake::new(Point::new(width - 2, height / 2), Direction::Left);
        snake.expand_head(snake.direction);
        players.push(Player::new(snake, robot2));
    }
