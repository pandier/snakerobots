...
pub async fn exists_robot(app: &AppState, user_id: Uuid, id: impl TryInto<Uuid>) -> ServiceResult<bool> {
    let Ok(id) = id.try_into() else { return Ok(false); };
    Ok(sqlx::query("SELECT id FROM robots WHERE id = $1 AND user_id = $2")
        .bind(id)
        .bind(user_id)
        .fetch_optional(&app.pg)
        .await?
